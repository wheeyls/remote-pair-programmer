...
function extractFileDirectives(text) {
  ...
  if (addFilesMatch && addFilesMatch[1]) {
    ...
    if (addFilesList) {
      result.addFiles = addFilesList.map(item => item.replace(/^-\s+/, '').trim());
    }
  }
  ...
  if (ignoreFilesMatch && ignoreFilesMatch[1]) {
    ...
    if (ignoreFilesList) {
      result.ignoreFiles = ignoreFilesList.map(item => item.replace(/^-\s+/, '').trim());
    }
  }
 ...
 * @returns {Object} - Object mapping file paths to their contents
 */
function processFileContext(text, baseFiles = []) {
  // Extract directives
  const directives = extractFileDirectives(text);
  
  // Resolve add-files globs
  const additionalFiles = resolveGlobs(directives.addFiles);
  
  // Combine base files with additional files
  let allFiles = [...baseFiles, ...additionalFiles];
